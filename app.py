...
class FitnessTracker:
    ...
    def add_afternoon_entry(self, entry_date, calories, dumps, protein, carbs, fats,
        ...
        if not existing_entries_loc.empty:
            for replace_loc in existing_entries_loc.values:
                ...
                st.session_state.entries.loc[replace_loc,"What happened?"] = what_happened_text
            ...
            st.session_state.entries = pd.concat([st.session_state.entries,pd.DataFrame(
                ...
                "What happened?":what_happened_text,
    ...
    def add_entry_night_form(self):
        ...
        # Create a form with user input options for daily activities and things
        accomplishments_form = st.form("Daily_Activities_Form")
        # Add user input options for daily activities and things within the form
        with accomplishments_form:
            new_accomplishment = st.text_input("Add New Daily Activity or Thing")
            if new_accomplishment:
                new_accomplishment = [new_accomplishment]
            accomplishments_input = st.multiselect("Daily Activities and Things", options=[
                "Workout","Cardio","Soccer","Mobility","Spanish","Japanese","Coding Project",
                "Books","Reading Research","Sick","Sunshine","Creatine","Vitamins","Smoke","Drink","Date",";)","Video Games"])
            accomplishments_input.extend(new_accomplishment)
